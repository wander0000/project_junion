...
public class ResumeUploadController {
	...
	@PostMapping("/resumeUploadAjaxAction")
//	public void uploadAjaxPost(MultipartFile[] uploadFile) {
//	ResponseEntity : 파일 정보를 넘기기위해서 사용
	public ResponseEntity<List<ResumeUploadDTO>> uploadAjaxPost(MultipartFile[] uploadFile) {
		log.info("upload ajax post...");
		
		List<ResumeUploadDTO> list = new ArrayList<>();
...
//			boardAttachDTO.setFilename(uploadFileName);
			resumeUploadDTO.setFileName(uploadFileName);
			resumeUploadDTO.setUuid(uuid.toString());
//			boardAttachDTO.setUploadpath(uploadFolderPath);
			resumeUploadDTO.setUploadPath(uploadFolderPath);
			log.info("@# resumeUploadDTO 01=>"+resumeUploadDTO);
...
//				참이면 이미지 파일
				if (checkImageType(saveFile)) {
					resumeUploadDTO.setImage(true);
					log.info("@# resumeUploadDTO 02=>"+resumeUploadDTO);
...
//		파일정보들을 list 객체에 담고, http 상태값은 정상으로 리턴
		return new ResponseEntity<List<ResumeUploadDTO>>(list, HttpStatus.OK);
	}
...
//	이미지파일을 받아서 화면에 출력(byte 배열타입)
	...
	public ResponseEntity<byte[]> getFile(String fileName) {
		log.info("@# resumeDisplay fileName=>"+fileName);
...
//			파일정보를 byte 배열로 복사+헤더정보+http상태 정상을 결과에 저장
	...
	@PostMapping("/resumeDeleteFile")
	public ResponseEntity<String> deleteFile(String fileName, String type) {
		log.info("@# resumeDeleteFile fileName=>"+fileName);
		File file;
...
//	@ResponseBody
	@GetMapping(value = "/resumeGetFileList")
	public ResponseEntity<List<ResumeUploadDTO>> getFileList(@RequestParam HashMap<String, String> param) {
		log.info("@# getFileList()");
		log.info("@# param=>"+param);
		log.info("@# param=>"+param.get("resume_num"));
		
		return new ResponseEntity<>(service.resumeGetFileList(Integer.parseInt(param.get("resume_num"))), HttpStatus.OK);
	}
